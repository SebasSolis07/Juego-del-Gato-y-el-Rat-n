...
def movimiento_usuario(posicion_raton, dim):
    ...
    # Si el usuario se sale del rango no pasa nada ni le avisa solo se queda en su posicion
...
def distanciamanhattan(posicion_raton, posicion_gato):
...
# La profundidad para mi representa solos los posible movimientos del  gato que le acerca al raton     
def minimax(posicion_gato, posicion_raton, dim, profundidad, turno_gato):
    ...
    if profundidad == 0 or posicion_gato == posicion_raton:
        valor = distanciamanhattan(posicion_raton, posicion_gato)
    ...
    if turno_gato:
        ...
        #Aplico recursividad. Donde evaluo la posGato y el pos raton
        for mov in lista_movimientos:
            ...
            # De regreso de la subrutina tengo el vector posicion del gato y el valor del resultado entre coordenadas
            if valor < mejor_valor:  
                #Esta variable cambian de acuerdo al mejor resultado que minimiza al raton
                mejor_valor = valor
...
'''
Pseudocódigo
Definir la dimension de la matriz, pos del gato, pos del raton.  
Generar una sub  rutina que imprima el tablero 
Realizar una evualacion de posiciones 
Aplicar el minimax,  recolectando en un vector las posibles posiciones del gato y cual es más conveniente para acercarce al raton
